...
<div class="form-group">
    ...
    <div class="form-control-wrap">
        ...
        <input type="number" @if($required !='' && $required=='true' ) required="{{$required}}" @endif data-parsley-errors-container=".parsley-container-{{ $name }}" id="{{$for}}" min="{{$min}}" value="{{$value}}"  name="{{ $name }}"  {{ $attributes }} {{ $attributes->merge(['class' => 'form-control ' . $class]) }} />
    </div>
