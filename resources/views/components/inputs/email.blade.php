...
<div class="form-group">
    ...
    <div class="form-control-wrap">
        ...
        <input type="email" @if($required !='' && $required=='true' ) required="{{$required}}" @endif data-parsley-errors-container=".parsley-container-{{ $name }}" id="{{$for}}" value="{{$value}}"  name="{{ $name }}"  maxlength="{{ $maxlength }}" {{ $attributes }} {{ $attributes->merge(['class' => 'form-control ' . $class]) }} autocomplete="off"/>
    </div>
