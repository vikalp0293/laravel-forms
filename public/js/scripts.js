...
!function (NioApp, $) {
	...
	NioApp.CurrentLink = function () {
		...
		function filterByLink(arr, url, pop) {
			var the_arr = url.split('/');
			var new_arr = the_arr.slice(0, the_arr.length - pop);
			var FUrl = new_arr.join('/');
			return arr.filter(name => name.includes(FUrl))[0]
		}
		...
		$(_link).each(function () {
			...
			if (fileName == _self_link) {
				setActive(self)
				childestLink.push(false)
			} else {
				self.closest("li").removeClass('active current-page').parents().closest("li:not(.current-page)").removeClass("active");
				childestLink.push(true)
			}
		});

		if (childestLink.every(val => val === true)) {
			if (filterByLink(links, _currentURL, 1)) {
				var lastActive = $('[href="' + filterByLink(links, _currentURL, 1) + '"]')
				setActive(lastActive)
			} else if (filterByLink(links, _currentURL, 2)) {
				var lastActive = $('[href="' + filterByLink(links, _currentURL, 2) + '"]')
				setActive(lastActive)
			}
		}
	...
	NioApp.TGL.content = function (elm, opt) {
		...
		$doc.on('mouseup', function (e) {
			if (toggleCurrent) {
				...
				if (!$toggleCurrent.is(e.target) && $toggleCurrent.has(e.target).length === 0 && !$contentD.is(e.target) && $contentD.has(e.target).length === 0 && !$(e.target).hasClass('select2-results__option')) {
					if (attr.onCloseCallback) {
						attr.onCloseCallback();
					}
			...
			$(elm).each(function () {
				...
				var def = {
					...
					language: {
						...
						sEmptyTable:'<div><img width="300" src="'+APP_BASE_URL+'/images/empty_item.svg" /></div><h5>No records found.</h5>'
					}
				},
					attr = opt ? extend(def, opt) : def;
				...
				return init[elm] = $(this).DataTable(attr);
