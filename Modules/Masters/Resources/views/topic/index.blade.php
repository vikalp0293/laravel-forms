...
@endphp
    <div class="nk-block-head nk-block-head-sm">
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                <h3 class="nk-block-title page-title">Topics</h3>
                @php 
                    if(count($topics) > 1) 
                        $count = 'topics';
                    else
                        $count = 'topic';                       
                @endphp
                <p>You have total <span class="record_count">{{ count($topics) }}</span> {{ $count }}.</p>
    ...
    <div class="nk-block table-compact">
        ...
        <form role="form" id="addForm" method="post" action="{{ url('masters/topic/add') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="nk-block">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-mb-12">
                        <label class="form-label" for="site-name">Subject<span class="text-danger">*</span></span></label>
                        <select size="sm" class="form-select form-control form-control-lg" data-placeholder="Subject" data-parsley-errors-container=".categoryParsley" name="subject_id" id="parentCat" data-search='on' required> 
                    ...
                    <div class="col-12">
                        <x-inputs.text for="name" label="Name" Drawer="true" icon="sign-dash" placeholder="Name" name="name" minlength="3" maxlength="200" autocomplete="off" required />
                    </div>

                    <div class="col-6">
                        <div class="form-group">
                            <label class="form-label" for="topic">Status</label>
...
<script type="text/javascript">
    ...
    function updateMassItems() {
        ...
        if(arr.length==0){
            ...
            $.ajax({
                url: root_url + '/masters/topic/mass-update',
    ...
    $(function() {
        ...
        $('.table-init').on('click', '.editItem', function() {
            ...
            $.ajax({
                url: root_url + '/masters/topic/get-topic',
                data: {'id':id},
                //dataType: "html",
                method: "GET",
                cache: false,
                success: function (data) {
                    ...
                    if(data.success){
                        ...
                        $('#name').val(data.topic.name);
                        $('#parentCat').val(data.topic.subject_id).trigger('change');
                        $('.submitBtn').text('Update');
                        
                        $('#manufStatus').attr('checked', data.topic.status == "active"? true : false);
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            $.ajax({
                url: root_url + '/masters/topic/delete',
                data: {
                    'id': id
                },
                //dataType: "html",
