...
class SubjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = Subject::whereNull('deleted_at')
        ...
        return view('masters::subject/index',['subjects'=>$data]);
    ...
    public function getSubject(Request $request)
    {
        ...
        if(!empty($subject->toArray())){
            return array('subject' => $subject,'success'=>true);
        }else{
            return array('success'=>false,'subject'=>array());
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                return redirect('masters/subject')->with('error', $messages);
            } else {
                ...
                if($request->input("id") && $request->input("id")!='0' && $request->input("id")!=''){
                    ...
                    $isExists = Subject::where('name',$request->input("name"))->get()->toArray();
                    
                    if(!empty($isExists)){
                        return redirect('masters/subject')->with('error', trans('messages.UNIT_TITLE'));
                    }
                ...
                $subject->status = $request->input("status")=='1' ? 'active' : "inactive";
                
                
                if($subject->save()){
                    return redirect('masters/subject')->with('message', $msg);
                }else{
                    return redirect('masters/subject')->with('error', trans('messages.SOMETHING_WENT_WRONG'));
                }
            }
