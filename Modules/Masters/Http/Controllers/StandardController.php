...
class StandardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = Standard::select('s.name as topic','m_standards.*')
                ...
                if (!empty($request->toArray())) {
                        if(isset($request->status) && (!empty($request->status) ) ){
                            $query->where('m_standards.status',$request->input('status'));
                        }

                        if(isset($request->name) && (!empty($request->name) || $request->name != 0)){
                            $query->where('m_standards.name',$request->input('name'));
                        }
                    }
                })
                ->orderBy('m_standards.id','DESC')
                ->get();      
        if ($request->ajax()) {
        ...
        return view('masters::standard/index',['standards'=>$data,'topics' => $topics]);
    ...
    public function getStandard(Request $request)
    {
        ...
        if(!empty($standard->toArray())){
            return array('standard' => $standard,'success'=>true);
        }else{
            return array('success'=>false,'standard'=>array());
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                return redirect('masters/standard')->with('error', $messages);
            } else {
                ...
                if($request->input("id") && $request->input("id")!='0' && $request->input("id")!=''){
                    ...
                    $isExists = Standard::where('name',$request->input("name"))->get()->toArray();
                    
                    if(!empty($isExists)){
                        return redirect('masters/standard')->with('error', trans('messages.UNIT_TITLE'));
                    }
                ...
                $standard->sub_topic_id = $request->input("sub_topic_id");
                $standard->status = $request->input("status")=='1' ? 'active' : "inactive";
                
                
                if($standard->save()){
                    return redirect('masters/standard')->with('message', $msg);
                }else{
                    return redirect('masters/standard')->with('error', trans('messages.SOMETHING_WENT_WRONG'));
                }
            }
    ...
    public function massUpdate(Request $request)
    {
        try {
            ...
            foreach($request->input("ids") as $key=>$value ){
                ...
                $standard->status = $request->input("status");
                $standard->save();
                $i++;
            }
    ...
    public function destroy(Request $request)
    {
        ...
        if ($item->delete()) {
            return array('standard' =>array(),'success'=>true,'msg'=>'success');
        }
        else{
            return array('success'=>false,'standard'=>array(),'msg'=>'fails');
        }
    }
}
